...
export async function loadTeacherChapters(profileId: string | null) {
  ...
  try {
    ...
    try {
      const { data: matData } = await supabase.from('matiere').select('id, nom, code');
      if (matData && matData.length > 0) {
        matieresList = matData;
      } else {
        const { data: matData2 } = await supabase.from('matieres').select('id, nom, code');
        if (matData2) matieresList = matData2;
      }
    } catch {}
