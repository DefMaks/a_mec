...
export function Sidebar() {
  ...
  if (isSuperAdmin || isAdmin) {
    ...
    sections = [
      {
        items: [
          { label: 'Espace Famille & Synthèse', href: '/', icon: LayoutDashboard },
        ],
      },
      {
        title: 'Admissions & Scolarité',
        items: [
          { label: 'Inscriptions & Admissions', href: '/parent/enrollment', icon: UserPlus, badge: 'Direct' },
          { label: 'Mes Enfants Inscrits', href: '/parent/children', icon: GraduationCap },
          { label: 'Minerval & Frais Twiga', href: '/parent/payments', icon: CreditCard },
        ],
      },
      {
        title: 'Communication',
  ...
  return (
    ...
    >
      ...
      {/* Role Switcher in Sidebar Footer - Visible uniquement si IS_PROD = false */}
      {!isProduction() && (
      ...
      {isProduction() && !isCollapsed && (
        <div className="p-3 border-t border-[#E2E8F0] bg-[#F8FAFC] text-[10px] text-[#64748B] text-center">
          <span>Portail Académie du Salut • RDC</span>
        </div>
      )}
    </aside>
